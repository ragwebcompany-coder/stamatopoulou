...
<meta name="author" content="{NAME}, {SPECIALTY}">
...
def logo_block(depth, footer=False):
    ...
    return (f'<a class="cb-logo{cls}" href="{r}index.html" aria-label="{BRAND} — αρχική">'
...
def nav_links(depth, active):
    ...
    for label, href in NAV:
        ...
        items = "".join(
            f'<a href="{r}{s["slug"]}">{icon(s["icon"], "w-5 h-5")}'
            f'<span><span class="cb-nav__dt">{s["short"]}</span>'
            f'<span class="cb-nav__dd">{s["who"]}</span></span></a>'
            for s in SERVICES
        )
        out.append(
            ...
            f'<span>ΟΛΕΣ ΟΙ ΥΠΗΡΕΣΙΕΣ</span><span>&#8594;</span></a>'
            f'</span></span></span>'
        )
    return "".join(out)
...
def header(depth, active):
    ...
    menu_links = "".join('<a href="%s%s">%s</a>' % (r, href, label) for label, href in NAV)
    svc_links = "".join('<a href="%s%s">%s</a>' % (r, s["slug"], s["nav"]) for s in SERVICES)
    return f"""<a class="cb-skip" href="#main">Μετάβαση στο περιεχόμενο</a>
<div class="cb-topbar">
...
<span class="cb-topbar__item cb-topbar__item--hours">{icon('clock','w-4 h-4')}<span>Κατόπιν ραντεβού</span></span>
<a class="cb-topbar__item cb-topbar__item--tel" href="tel:+30{PHONE}">{icon('phone','w-4 h-4')}<span>{PHONE_P}</span></a>
<a class="cb-topbar__item cb-topbar__item--tel" href="tel:+30{MOBILE}">{icon('phone','w-4 h-4')}<span>{MOBILE_P}</span></a>
</div>
</div>
...
<nav class="cb-nav" aria-label="Κύρια πλοήγηση">{links}</nav>
<a class="cb-btn cb-btn--sm" href="{r}epikoinonia.html#rantevou">ΡΑΝΤΕΒΟΥ</a>
</div>
<button class="lg:hidden cb-burger" aria-label="Άνοιγμα μενού" aria-controls="cb-menu" aria-expanded="false">
...
<button id="cb-menu-close" class="dln-overlay__close" aria-label="Κλείσιμο μενού">&#215;</button>
<nav class="dln-overlay__nav" aria-label="Πλοήγηση κινητού">
{menu_links}
<span class="dln-overlay__sep">Υπηρεσίες</span>
...
def footer_loop():
    ...
    return ('<div class="cb-footer__loop" aria-hidden="true">'
            ...
            'ΚΛΕΙΣΤΕ ΡΑΝΤΕΒΟΥ ΚΛΕΙΣΤΕ ΡΑΝΤΕΒΟΥ ΚΛΕΙΣΤΕ ΡΑΝΤΕΒΟΥ</div></div>')
...
<div>
<h2 class="cb-cta-band__title">Ας κάνουμε το πρώτο βήμα</h2>
<p class="cb-cta-band__lede">Μια πρώτη συνάντηση γνωριμίας {SESSION} λεπτών — στο γραφείο στην {CITY} ή διαδικτυακά.
Το ραντεβού κλείνεται με ένα τηλεφώνημα.</p>
</div>
<div class="cb-cta-band__actions">
<a class="cb-btn cb-btn--light" href="tel:+30{PHONE}">{icon('phone','w-4 h-4')}<span>{PHONE_P}</span></a>
<a class="cb-btn cb-btn--outline" href="{r}epikoinonia.html#rantevou">ΕΠΙΚΟΙΝΩΝΙΑ<span class="cb-btn__arrow">&#8594;</span></a>
</div>
</div>
</div>
</section>"""
...
def footer(depth):
    r = rel(depth)
    nav_links = "".join('<li><a href="%s%s">%s</a></li>' % (r, href, label) for label, href in NAV)
    svc_links = "".join('<li><a href="%s%s">%s</a></li>' % (r, s["slug"], s["nav"]) for s in SERVICES)
    areas = " · ".join(AREAS[:-1])
...
<p class="cb-footer__tag">Ψυχοθεραπεία παιδιών, εφήβων και ενηλίκων, συμβουλευτική γονέων και
παιγνιοθεραπεία, στην {CITY} ή διαδικτυακά.</p>
<p class="cb-footer__tag cb-footer__tag--sign">Κλινική Ψυχολόγος MSc · αρ. αδείας {LICENSE} ·
μέλος του Συλλόγου Ελλήνων Ψυχολόγων.</p>
</div>

<div class="cb-footer__col">
<h2 class="cb-footer__h">Πλοήγηση</h2>
<ul class="cb-footer__list">{nav_links}
<li><a href="{r}psychologos-ilioupoli.html">Ψυχολόγος {CITY}</a></li>
<li><a href="{r}syxnes-erotiseis.html">Συχνές Ερωτήσεις</a></li></ul>
</div>

<div class="cb-footer__col">
<h2 class="cb-footer__h">Υπηρεσίες</h2>
<ul class="cb-footer__list">{svc_links}</ul>
</div>

<div class="cb-footer__col">
<h2 class="cb-footer__h">Επικοινωνία</h2>
...
<li><a href="tel:+30{MOBILE}">{MOBILE_P}</a></li>
<li><a href="mailto:{EMAIL}">{EMAIL}</a></li>
</ul>
<p class="cb-footer__hours">Συνεδρίες αποκλειστικά κατόπιν ραντεβού.<br>Διά ζώσης ή διαδικτυακά.</p>
</div>

</div>

<p class="cb-footer__areas"><strong>Εξυπηρετούμε:</strong> {areas} και τα υπόλοιπα νότια προάστια —
καθώς και όλη την Ελλάδα διαδικτυακά.</p>

<div class="cb-footer__bottom">
<p>© 2026 {BRAND}. Με την επιφύλαξη παντός δικαιώματος.</p>
<p class="cb-footer__legal">
<a href="{r}politiki-aporritou.html">Πολιτική Απορρήτου</a>
<a href="{r}oroi-xrisis.html">Όροι Χρήσης</a>
<a href="{r}sitemap.xml">Χάρτης ιστότοπου</a>
</p>
...
<a class="cb-fab" href="tel:+30{PHONE}" aria-label="Καλέστε στο {PHONE_P}">
{icon('phone','w-5 h-5')}<span>Καλέστε μας</span></a>
...
def breadcrumbs(depth, trail):
    ...
    return ('<nav class="cb-crumbs" aria-label="Διαδρομή"><ol>%s</ol></nav>'
            % "".join(parts))
...
.cb-hero__h1 { font-family: var(--font-serif); font-weight: 300; letter-spacing: -.025em;
  font-size: clamp(2.4rem, 6.2vw, 4.5rem); line-height: 1.07; margin: 1.1rem 0 1.5rem; }
.cb-hero__lede { max-width: 46rem; font-weight: 300; font-size: 1.08rem; line-height: 1.8;
  color: rgba(248,247,252,.76); }
...
.cb-booking { background: #fff; border: 1px solid rgba(19,18,87,.09); border-radius: 4px; padding: 1.5rem;
  box-shadow: 0 24px 60px -44px rgba(19,18,87,.5); }
@media (min-width: 768px) { .cb-booking { padding: 2rem; } }
.cb-booking__ph { border: 2px dashed rgba(107,70,201,.32); background: rgba(107,70,201,.04);
  border-radius: 3px; padding: 3rem 1.75rem; text-align: center; display: flex; flex-direction: column;
  align-items: center; gap: .65rem; min-height: 24rem; justify-content: center; }
.cb-booking__ic { width: 2.75rem; height: 2.75rem; color: var(--cb-violet); opacity: .8; margin-bottom: .35rem; }
.cb-booking__t { font-family: var(--font-serif); font-size: 1.5rem; color: var(--cb-ink); }
.cb-booking__d { font-weight: 300; line-height: 1.7; color: var(--cb-muted); max-width: 42ch; }
.cb-booking__d--em { color: var(--cb-ink); margin-top: .9rem; }
.cb-booking__actions { display: flex; flex-wrap: wrap; gap: .75rem; justify-content: center; margin-top: .75rem; }
.cb-booking__note { margin-top: 1.4rem; padding-top: 1.2rem; border-top: 1px solid rgba(19,18,87,.1);
...
.cb-booking .calendly-inline-widget { min-width: 0 !important; width: 100%; }
...
.cb-hero__h1 { font-size: clamp(2rem, 7.4vw, 4.5rem); }
...
/* Σημείωση για τις συμπληρωματικές μεθόδους (Theta Healing / ραδιαισθησία).
